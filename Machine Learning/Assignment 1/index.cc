...
/*
 * Calculates the minkowski distance when p = 3
*/
void findMinkowskiDistance(vector<vector<float>> &data, int kVal){

    vector<pair<float, vector<float>>> distances;
    vector<float> newInstance = data[data.size() - 1];

    for(size_t i = 0; i < data.size() - 1; i++){
        float distVal = pow(abs(data[i][0] - newInstance[0]),3) + pow(abs(data[i][1] - newInstance[1]),3)+ pow(abs(data[i][2] - newInstance[2]),3) + pow(abs(data[i][3] - newInstance[3]),3); 
        distVal = cbrt(distVal);
...
/*
 * Calculates the chevyshev distance
*/
void findChebyshevDistance(vector<vector<float>> &data, int kVal){

 vector<pair<float, vector<float>>> distances;
    vector<float> newInstance = data[data.size() - 1];

    for(size_t i = 0; i < data.size() - 1; i++){
        float distVal = max( max(abs(data[i][0] - newInstance[0]), abs(data[i][1] - newInstance[1])), max(abs(data[i][2] - newInstance[2]), abs(data[i][3] - newInstance[3]))); 
        distances.push_back(pair<float, vector<float>> (distVal, data[i]));
    }    
    ...
    cout << "The class generated by Chevyshev distance for k = " << kVal << " is " << char(data[data.size() -1][4]) << endl;
...
//function that fins the maximum and minimum for a feature
...
// normalizes the features for all instances
...
int main(){
    ...
    // COMMENT THIS FUNCTION FOR STANDARD FORM
    ...
    findChebyshevDistance(data, kVal);
    
    findMinkowskiDistance(data, kVal);
